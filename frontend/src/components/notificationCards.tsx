...
const NotificationCards = ({ isOpen, onClose, title, notificationIcon, description, additionalText }: NotificationModalProps) => {
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md p-3 lg:p-8">
        <DialogHeader className="text-start space-y-4">
          <DialogTitle className="flex items-center gap-4 text-lg lg:text-xl text-center font-normal mt-5 text-black">
            {notificationIcon}
            {title}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4 mt-2 lg:mt-6">
          {description && (
