...
import { getDataWithToken, postData } from "../lib/apiMethods";
...
const UserLogin = () => {
  ...
  const handleNext =  async (data: LoginFormData) => {
    ...
    try {
      ...
      if (resp.status === 200) {
        const { access, refresh, is_owner, is_active } = resp?.data;
  
        if (!access || !refresh) {
          throw new Error("No access or refresh token received from server");
        }

        sessionStorage.setItem('temp_verification_token', access);
  ...
  return (
    <>
      <EmailVerificationModal isOpen={showVerificationModal} onClose={() => setShowVerificationModal(false)} userEmail={watch("email")} accessToken={token || ''} />
      ...
      <div className="flex flex-col md:flex-row gap-6 md:gap-8 justify-between items-start h-full">
        ...
        <form onSubmit={handleSubmit(handleNext)} className="w-full">
          ...
          <div className="mb-4 w-full flex justify-between items-center">
            ...
            <a href="/password-reset" className="text-xs md:text-sm text-[#001EB4] font-medium hover:underline">Forgot password?</a>
          </div>
